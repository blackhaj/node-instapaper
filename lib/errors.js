...
const errors = [
  ...
  { code: 1220, name: 'DomainError', message: 'Domain requires full content to be supplied' },
  {
    code: 1221,
    name: 'DomainOptedOut',
    message: 'Domain has opted out of Instapaper compatibility',
  },
  ...
  {
    code: 1244,
    name: 'MissingProgressTimestamp',
    message: 'Invalid or missing progress_timestamp',
  },
  {
    code: 1245,
    name: 'PrivateBookmarkError',
    message: 'Private bookmarks require supplied content',
  },
...
errors.forEach((error) => {
  const err = customError(error.name).bind(error.message);
